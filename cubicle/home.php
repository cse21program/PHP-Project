...
if(!isset($_SESSION['id'])) {
    ...
    $query3 = "SELECT * FROM `club_members` WHERE student_id ='$current_user_id'";
	$post_row = mysqli_fetch_array(mysqli_query($con, $query3), MYSQLI_ASSOC);

    $current_user_info = mysqli_query($con, $query2);
    $row2 = mysqli_fetch_array($current_user_info, MYSQLI_ASSOC);
    $current_user_name = $row2['student_name'];
    $current_user_profile_picture = $row2['profile_picture'];
	$query4  = "SELECT * FROM club_details WHERE club_id = '$cc_club'";
	$post_row2 = mysqli_fetch_array(mysqli_query($con, $query4));
	$cc_profile_picture = $post_row2['profile_picture'];
	$cc_name = $post_row2['club_name'];
?>
...
<?php 
	...
	include('functions.php');
		...
		$mm_club_id = $row['club_id'];
		$location = $row['post_image'];
		$time = $row['time'];
		$post_id = $row['post_id'];
		$content = $row['content'];

		$query5 = mysqli_query($con, "SELECT * FROM club_members WHERE club_id = '$mm_club_id' and  student_id = '$current_user_id'");
		while($row3 = mysqli_fetch_array($query5, MYSQLI_ASSOC)) {
			$mm_club_profile_picture = $row3['club_profile_picture'];
			$mm_club_name = $row3['club_name'];
...
<div class="card gedf-card">
	<div class="card-header">
		<div class="d-flex justify-content-between align-items-center">
			<div class="d-flex justify-content-between align-items-center">
				<div class="mr-2">
					<img class="rounded-circle" width="45" src="<?php echo "admin/".$mm_club_profile_picture?>" alt="">
				</div>
				<div style="margin-left: 10px" class="ml-2">
					<div class="h6 m-0"><?php echo $mm_club_name?></div>
				</div>
			</div>
	...
	<div class="justify-content-center card-body">
		...
		<img width=90% height=350px style=" display: block;margin-left: auto;margin-right: auto;" src="<?php echo "admin/".$location?>" alt="">
	</div>
	<div class="card-footer text-center ">
	<?php
	session_start();
		$like = mysqli_query($con, "SELECT * FROM post_like WHERE id = '$current_user_id' and post_id = '$post_id'");
